...
private:
    Font();
    ~Font();
    std::map<std::string, Texture*> buffer_;    //缓存所有已经画过的字体
    Texture* indexTex(const std::string& fontname, uint16_t c, int size);
...
public:
    ...
    void draw(const std::string& fontname, const std::string& text, int size, int x, int y, Color color = { 255, 255, 255, 255 }, uint8_t alpha = 255);
    void clearBuffer();
    int getBufferSize() { return buffer_.size(); }

    Texture* createTextTexture2(const std::string& fontname, const std::string& s, int size);
